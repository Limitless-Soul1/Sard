...
/// Presentation sub-switches, same convention (absent = on). Enforced in the CORE too, not only
/// the UI: the chapter label is book content, so a stale frontend must not leak it either.
...
/// Which activity an update describes. The presentation sub-switches only apply to READING
/// activities — the browsing line (app name) is never stripped by them.
#[derive(Deserialize, Debug, Default, PartialEq, Eq)]
#[serde(rename_all = "camelCase")] // the frontend sends `kind: "reading"` / `"browsing"`
...
pub struct PresenceActivity {
    /// The kind of activity — see `ActivityKind`.
    #[serde(default)]
    pub kind: ActivityKind,
    /// The book's title — the details line carries `"Reading {title}"`, composed by the frontend.
    pub details: String,
    /// The position line: chapter label + whole-book percent (e.g. `"Chapter 3 · 45%"`), or just
    /// the percent when the engine has no chapter label. Composed by the frontend; while no book
    /// is open it carries the localized browsing line instead.
    #[serde(default)]
...
/// The presentation sub-switches (`discord_rpc_show_*`) are enforced here too, so their privacy
/// promise holds even against a stale frontend: a reading activity whose book line or position
/// line is switched off is stripped BEFORE it reaches the worker — and thus never the pipe.
#[tauri::command]
pub fn presence_update(
    ...
    let off = |key: &str| {
        settings::get(&conn, key)
            ...
            .map(|v| v.as_deref() == Some("0"))
    };
    if off(RPC_SETTING_KEY)? {
        return Ok(());
    }
    let mut activity = activity;
    if activity.kind == ActivityKind::Reading {
        if off(SHOW_BOOK_KEY)? {
            activity.details.clear();
        }
        if off(SHOW_POSITION_KEY)? {
            activity.state = None;
        }
    }
