...
| ~~**PPC-4**~~ | ~~Typography sliders do not mirror in RTL~~ | **CLOSED 2026-08-07.** The filed risk was wrong: no pin needed undoing. The settings drawer was the ONLY reader panel not carrying `dir` (Contents/Notes/photo tray all do), so it inherited the LTR pin RAWY-89 puts on `.reader-root`. Measured: slider `ltr`→`rtl`, small A moved from left to right, drawer right edge unmoved at the viewport edge, `.reader-root` still `ltr`, chapters drawer still docked left with no collision. `npm run harness:rtl`. |
