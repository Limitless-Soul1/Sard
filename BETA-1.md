...
3. **Typography sliders read left-to-right in Arabic.** Small `A` on the left, large `A` on the
   right, while the volume slider mirrors correctly. **This is intentional and will not change**
   (owner, 2026-08-07) — a UI preference, not a defect. Guarded by `npm run harness:panel-ltr` so
   it is not re-filed as a bug. See PPC-4.
...
| ~~**PPC-2**~~ | ~~Byte-identity has only ever measured **scrolled** rendering~~ | **CLOSED 2026-08-07.** The gap was measured, not argued: across all 16 scrolled baseline books `layout.columns` was **1 — every single one**, so the field added to catch a pagination collapse was a constant and NAV-1 produced no difference at all. A PAGED baseline now exists (`paged-1`, 13 books, 12 of them with columns>1). FALSIFIED both ways: reintroducing NAV-1 made the compare fail on every book (`columns 418 → 1`, `pages 508 → 3`); restoring the fix returned it to byte-identical. `--flow=paged` makes the mode explicit instead of inherited from the profile. |
| ~~**PPC-3**~~ | ~~FINDING-5 — الشوقيات reports `paras: 1`~~ | **CLOSED 2026-08-07 — no defect; it was the NAME.** The field counted `<p>` elements while calling them paragraphs. Measured on section 5 (21,076 chars): 50 `<p>` but **805 elements carrying their own text**, and the reading pipeline resolves **104 blocks with 104 usable DOM ranges**, covering 100% of the text. The count was right; the name was a claim it could not support. `byte-identity` renamed `paragraphs`→`pTags` (css-modes.mjs already had). `npm run harness:ppc3`. ⚠ Separately: this book is NOT in the library, so byte-identity skips it on every run — the one book the finding is about has been outside the net entirely. |
| **PPC-4** | ~~Typography sliders do not mirror in RTL~~ → **NOT A DEFECT — intended behaviour** | **DECIDED 2026-08-07 (owner).** The LTR slider layout is deliberate and stays. A change to mirror the drawer was implemented and then **reverted at the owner's instruction**: it is a UI preference, not a bug, and was never approved. `SettingsPanel.tsx` is byte-identical to its pre-change state. The decision is now guarded by `npm run harness:panel-ltr`, which FAILS if the drawer is ever made to follow the UI language again — because the reason PPC-4 was filed at all is that this intent was written down nowhere. |
| ~~**PPC-5**~~ | ~~Adopt `resilience-1-final` as the byte-identity baseline~~ | **ADOPTED 2026-08-07.** BOTH are official now: `resilience-1-final` (scrolled) and `paged-1` (paged). Checking only the scrolled one would re-open the hole PPC-2 closed — `columns` is inert in scrolled mode. `resilience-1-final` was RE-CAPTURED on adoption rather than adopted as it stood: its old content had drifted against the current reading settings (pageOpacity 1→0.84, zoom 2→2.5, hide_chapter_titles 1→0, hide_first_line 1→0), each flagged by the harness config guard as "not a code change — re-baseline". The pre-adoption capture is preserved in git (18b042e). `npm run harness:fingerprint:all`. |
| **PPC-6** | English UI layout audit | **DEFERRED** (owner, 2026-08-07). Still blocked on a screenshot of the English UI; to be done closer to Feature Freeze. |
| **PPC-7** | Backfill metadata for pre-existing library rows | **DEFERRED to after Feature Freeze** (owner, 2026-08-07). A migration that touches real user data must not run while the tree changes daily. Revisit in the cleanup phase. |

| **FUTURE-1** | الشوقيات is not in the library, so `byte-identity` skips it on every run | Found while closing PPC-3 (2026-08-07). The one book that finding was about has been outside the safety net the whole time — `skipped — not in the library` on every capture. Adding it is a corpus decision, deliberately not taken during active development. |
