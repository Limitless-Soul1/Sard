...
export const CONTENT_RULES = [
  ...
  {
    id: "signing-material",
    why: "code-signing configuration or a reference to a signing credential",
    // A SEPARATE RULE FROM `secret`, because the shapes genuinely differ and the existing one cannot
    // see these. `secret` requires a quoted value (`password: "…"`); a Gradle properties file writes
    // `storePassword=hunter2` with no quotes at all, and an Xcode project writes `DEVELOPMENT_TEAM =
    // A1B2C3D4E5` — neither matches, and both are exactly what a generated mobile project contains.
    //
    // Narrow by construction: every token below is a signing-specific identifier or file extension
    // that has no ordinary meaning in this codebase, so the rule describes one specific thing and
    // cannot fire on product prose. It is a CONTENT backstop to the path rule in
    // production-tree-rules.mjs — that rule keeps the generated projects out; this one catches a
    // signing reference copied into a file that legitimately ships.
    re: /keystore\.properties|storePassword|keyPassword|keyAlias|\.(?:jks|keystore|p12|mobileprovision|provisioningprofile)\b|ExportOptions\.plist|DEVELOPMENT_TEAM|PROVISIONING_PROFILE_SPECIFIER|CODE_SIGN_IDENTITY|ASC_(?:KEY_ID|ISSUER_ID)/,
    exempt: [
      // Same reasoning as the personal-data and test-infrastructure rules above: the ignore file must
      // NAME the artifacts it refuses, and it is the local half of the protection. Sanitising it to
      // quieten this rule would delete the thing that stops the material being committed at all.
      { re: /^\.gitignore$/, why: "ignore patterns are paths, and they are the protection against committing signing material" },
    ],
  },
  {
    id: "attribution",
    ...
    exempt: [
      // AGENTS.md is addressed TO agents; it is not a claim about who wrote Sard. Its single match is
      // the phrase "AI-assisted development tool" in the sentence naming who must follow the rules —
      // the rule's subject appearing as the document's audience, which is the same reason the rule
      // files themselves are excused from the patterns they define. Rewording it to dodge the scanner
      // would leave the instructions addressed to nobody.
      //
      // Scoped to this one path and to THIS rule only. AGENTS.md is still scanned for private paths,
      // personal data, development tooling, internal documents, test infrastructure and secrets, and
      // no other file gains anything from this entry. In particular the rule still rejects a vendor
      // name or a co-authorship trailer anywhere in the tree, INCLUDING inside AGENTS.md's
      // neighbours — what is excused is one file, not one phrase.
      { re: /^AGENTS\.md$/, why: "the agent rules address agents; the match is their audience, not an attribution" },
    ],
  },
];
