...
[`WORKFLOW.md`](docs/WORKFLOW.md) is the authoritative repository workflow. This file adds obligations that
...
- AI agents **MUST NOT** create pull requests targeting `main`. `main` is not a merge target — it is a
  published snapshot produced by the release process in [`WORKFLOW.md`](docs/WORKFLOW.md).
...
easier to test.** See §11 and the `private/` rule in [`WORKFLOW.md`](docs/WORKFLOW.md).
...
This must remain consistent with the production-boundary rules in [`WORKFLOW.md`](docs/WORKFLOW.md): the
...
[`WORKFLOW.md`](docs/WORKFLOW.md) remains the authoritative repository workflow for branches, releases,
